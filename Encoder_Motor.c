...
long Present_time[4] = {0 ,0 ,0 ,0};     //LF ,LB ,RF ,RB
long Past_time[4] = {0 ,0 ,0 ,0};        //LF ,LB ,RF ,RB
...
float getRPM_TIM_Wheel(TIM_HandleTypeDef *TIM ,uint8_t _Wheel){
	...
	Present_time[_Wheel] = HAL_GetTick();

	_RPM[_Wheel] = (float)((int16_t)(Present_Count[_Wheel] - Past_Count[_Wheel])/(Present_time[_Wheel] - Past_time[_Wheel]));
	_RPM[_Wheel] = (float)(((_RPM[_Wheel]* 60.0f)/CPR )*1000.0f /Gear_Ratio);

	Past_Count[_Wheel] = Present_Count[_Wheel];
	Past_time[_Wheel] = Present_time[_Wheel];
